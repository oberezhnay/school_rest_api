...
router.get('/:id', async (req, res, next) => {
    try {
        const teacher = await teachertController.getTeacher(req.params.id)
        res.json(teacher)
    } catch(e) {
        res.status(500).json({message: 'Something go wrong, try again'})
    } finally {
...
router.put('/:id', async (req, res, next) => {
    try {
        const updatedTeacher = await teacherController.updateTeacher(req.params.id, req.params.body)
